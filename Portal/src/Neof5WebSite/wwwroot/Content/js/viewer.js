 ...
 * viewer.js
 * Web大富豪レコードビューワ
 * Copyright (C) 2014 NEOGENIA Ltd.
 */
enchant();

window.onload = function () {
    // ============================== Variables ==============================
    var gRecordId = document.getElementById('recordId').value;
    var gMainPlayerNum = 0;
    var gOpenDeckFlag = false;
...
//                    } else {
                    	if (i == this.mainPlayerNum || gOpenDeckFlag) {
                    		player.deck.replaceCards(deckObj[i]);
                    	} else {
...
//                    }
        ...
        var mainScene = function() {
            ...
            // 視点回転ボタン
            {
            	var btn = new Button("視点", "light", 22, 26);
            	btn.x = GAME_W - 146;
            	btn.y = GAME_H - 105;
            	btn.ontouchend = rotateView;
            	scene.addChild(btn);
            }

            // 手札丸見えボタン
            {
            	var btn = new Button("手札", "light", 22, 26);
            	btn.x = GAME_W - 98;
            	btn.y = GAME_H - 105;
            	btn.ontouchend = openDeckToggle;
            	scene.addChild(btn);
            }

            // 自動再生ボタン
            var autoPlaying = false;
            var btnAutoPlay = new Button("自動", "light", 22, 26);
            btnAutoPlay.x = GAME_W - 50;
            btnAutoPlay.y = GAME_H - 105;
            btnAutoPlay.ontouchend = autoPlayToggle;
            scene.addChild(btnAutoPlay);
            
        	// 先頭ボタン
            {
            	var btn = new Button("|<<", "light", 22, 26);
            	btn.ontouchend = backToTop;
            	scene.addChild(btn);
            	btn.x = GAME_W - 146;
            	btn.y = GAME_H - 75;
            }
            ...
            {
            	var options = {};
            	for (var i = 1; i <= document.getElementById('rounds').value; i++) {
            		options[i] = '第' + i + '回戦';
            	}
            	var selectbox = new InputSelectBox(options);
            	selectbox.onchange = function () {
            		reloadDataAtRound(selectbox.selected);
            	};
            	scene.addChild(selectbox);
            	selectbox.x = GAME_W - selectbox.width - 3;
            	selectbox.y = GAME_H - selectbox.height - 20;
            }
       /*
            // 開始ボタン
		...
		*/
			...
			function forward() {
				stopAutoPlay();
				autoPlaying = false; btnAutoPlay.text = '自動';
				if (gRecordPointer >= gRecordData.length - 1) return false;

				var last = gRecordData.Data[gRecordPointer];
				var msg = gRecordData.Data[++gRecordPointer];

				onMessage(msg);

				var wait_msec = 200;
				if (msg.Kind != 'CardsArePut' || msg.Ba.length == last.Ba.length) {
					if (msg.Kind == 'CardsArePut') wait_msec = 700;
					setTimeout(forward, wait_msec);
					return true;
				}
				return false;
			}
			...
			function backward() {
				stopAutoPlay();
				if (gRecordPointer <= 1) return false;

				var msg = gRecordData.Data[--gRecordPointer];
				var last = gRecordData.Data[gRecordPointer-1];

				var wait_msec = 1;
				if (msg.Kind == 'CardsArePut' || msg.Kind == 'Think') {
					wait_msec = 600;
					onMessage(msg);
				}
				if (msg.Kind != 'CardsArePut' || msg.Ba.length == last.Ba.length) {
					setTimeout(backward, wait_msec);
					return true;
				}
				return false;
			}
			...
			function autoPlay() {
				if (autoPlaying && gRecordPointer < gRecordData.Data.length - 1) {
					var last = gRecordData.Data[gRecordPointer];
					var msg = gRecordData.Data[++gRecordPointer];
					...
					var wait_msec = 200;
					if (msg.Kind == 'CardsArePut') wait_msec = 800;
					setTimeout(autoPlay, wait_msec);
				} else {
					stopAutoPlay();
				}
			}
			...
			function rotateView() {
				stopAutoPlay();
				gMainPlayerNum++;
				if (gMainPlayerNum >= 5) gMainPlayerNum = 0;
				pm.clear();
				pm.players.length = 0;
				onMessage(gRecordData.Data[gRecordPointer]);
			}

        	// 手札丸見え
			function openDeckToggle() {
				gOpenDeckFlag = !gOpenDeckFlag;
				if (!autoPlaying) {
					onMessage(gRecordData.Data[gRecordPointer]);
				}
			}
        	...
        	// ラウンド変更
			function reloadDataAtRound(n) {
				stopAutoPlay();
				$.ajax({
            		url: '/Content/records/'+gRecordId+'_'+n+'.js',
            		//url: '/Content/records/test.js',
            		dataType: 'json',
					cache: true,
            		...
            		error: function (XMLHttpRequest, textStatus, errorThrown) {
            			alert('Data download error ! ' + textStatus);
            		}
				});
			}
...
*/
			function onMessage(obj){
                if (obj.Kind == "Exception") {
                	...
                	var recordTitle = gRecordData.Header.Title + ' 第' + gRecordData.Header.Round + '回戦';
                	gameInfo.text = 'ゲーム開始';
                	showToast(recordTitle);
                	yama.clear();
                	pm.clear();
                } else {
                	...
                	if (!pm.players.length) {
                		...
                		pm.addPlayers(obj.PlayerInfo, gMainPlayerNum);
                		pm.doLayout();  // 手札の配置
                	}
